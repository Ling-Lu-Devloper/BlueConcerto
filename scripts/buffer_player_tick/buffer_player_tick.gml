function buffer_player_tick() {
	//Fill audio queue
	var buff = buffer_create(buff_size, buffer_fixed, buffer_wrap);

	//FIX FOR BUFFER BUG IN RECENT RUNTIMES
	repeat(buff_size) buffer_write(buff, buffer_u8, 0);

	var rendered_sample = aviz_tick(ogg, string(buffer_get_address(buff)), 0, tick_size);
